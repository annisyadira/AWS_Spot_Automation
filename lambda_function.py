...
def lambda_handler(event, context):
    ...
    # get the last 5 minute of vCPU usage from CloudWatch
...
def adjustment(z, vcpu_count):
    ...
    if new_vcpu == 2:
        ...
        price_on_each_az = [float(price_check_a['SpotPriceHistory'][0]['SpotPrice']), float(price_check_b['SpotPriceHistory'][0]['SpotPrice']), float(price_check_c['SpotPriceHistory'][0]['SpotPrice'])]
...
def selection2():
    ...
    alt1['discount(%)'] = (0.0066 - float(spot_price_1['SpotPriceHistory'][0]['SpotPrice'])) / 0.0066 * 100
    alt1['price'] = float(spot_price_1['SpotPriceHistory'][0]['SpotPrice'])
    alt2['discount(%)'] = (0.0066 - float(spot_price_2['SpotPriceHistory'][0]['SpotPrice'])) / 0.0066 * 100
    alt2['price'] = float(spot_price_2['SpotPriceHistory'][0]['SpotPrice'])
    alt3['discount(%)'] = (0.0066 - float(spot_price_3['SpotPriceHistory'][0]['SpotPrice'])) / 0.0066 * 100
    alt3['price'] = float(spot_price_3['SpotPriceHistory'][0]['SpotPrice'])
    alt4['discount(%)'] = (0.0123 - float(spot_price_4['SpotPriceHistory'][0]['SpotPrice'])) / 0.0123 * 100
    alt4['price'] = float(spot_price_4['SpotPriceHistory'][0]['SpotPrice'])
    alt5['discount(%)'] = (0.0123 - float(spot_price_5['SpotPriceHistory'][0]['SpotPrice'])) / 0.0123 * 100
    alt5['price'] = float(spot_price_5['SpotPriceHistory'][0]['SpotPrice'])
    alt6['discount(%)'] = (0.0123 - float(spot_price_6['SpotPriceHistory'][0]['SpotPrice'])) / 0.0123 * 100
    alt6['price'] = float(spot_price_6['SpotPriceHistory'][0]['SpotPrice'])
    alt7['discount(%)'] = (0.0264 - float(spot_price_7['SpotPriceHistory'][0]['SpotPrice'])) / 0.0264 * 100
    alt7['price'] = float(spot_price_7['SpotPriceHistory'][0]['SpotPrice'])
    alt8['discount(%)'] = (0.0264 - float(spot_price_8['SpotPriceHistory'][0]['SpotPrice'])) / 0.0264 * 100
    alt8['price'] = float(spot_price_8['SpotPriceHistory'][0]['SpotPrice'])
    alt9['discount(%)'] = (0.0264 - float(spot_price_9['SpotPriceHistory'][0]['SpotPrice'])) / 0.0264 * 100
    alt9['price'] = float(spot_price_9['SpotPriceHistory'][0]['SpotPrice'])
    alt10['discount(%)'] = (0.0528 - float(spot_price_10['SpotPriceHistory'][0]['SpotPrice'])) / 0.0528 * 100
    alt10['price'] = float(spot_price_10['SpotPriceHistory'][0]['SpotPrice'])
    alt11['discount(%)'] = (0.0528 - float(spot_price_11['SpotPriceHistory'][0]['SpotPrice'])) / 0.0528 * 100
    alt11['price'] = float(spot_price_11['SpotPriceHistory'][0]['SpotPrice'])
    alt12['discount(%)'] = (0.0528 - float(spot_price_12['SpotPriceHistory'][0]['SpotPrice'])) / 0.0528 * 100
    alt12['price'] = float(spot_price_12['SpotPriceHistory'][0]['SpotPrice'])
    alt13['discount(%)'] = (0.1056 - float(spot_price_13['SpotPriceHistory'][0]['SpotPrice'])) / 0.1056 * 100
    alt13['price'] = float(spot_price_13['SpotPriceHistory'][0]['SpotPrice'])
    alt14['discount(%)'] = (0.1056 - float(spot_price_14['SpotPriceHistory'][0]['SpotPrice'])) / 0.1056 * 100
    alt14['price'] = float(spot_price_14['SpotPriceHistory'][0]['SpotPrice'])
    alt15['discount(%)'] = (0.1056 - float(spot_price_15['SpotPriceHistory'][0]['SpotPrice'])) / 0.1056 * 100
    ...
    array_price.extend([alt1['price'], alt2['price'], alt3['price'], alt4['price'], alt5['price'], alt6['price'], alt7['price'], alt8['price'], alt9['price'], alt10['price'], alt11['price'], alt12['price'], alt13['price'], alt14['price'], alt15['price']])
    array_discount.extend([alt1['discount(%)'], alt2['discount(%)'], alt3['discount(%)'], alt4['discount(%)'], alt5['discount(%)'], alt6['discount(%)'], alt7['discount(%)'], alt8['discount(%)'], alt9['discount(%)'], alt10['discount(%)'], alt11['discount(%)'], alt12['discount(%)'], alt13['discount(%)'], alt14['discount(%)'], alt15['discount(%)']])
    
    array_memory_new = [0.625, 0.625, 0.625, 0.125, 0.125, 0,125, 0.25, 0.25, 0.25, 0.5, 0.5, 0.5, 1, 1, 1]
    array_price_new = []
    array_discount_new = []
    ...
    array_price_new.extend([1 - (array_price[0] / max_price), 1 - (array_price[1] / max_price), 1 - (array_price[2] / max_price), 1 - (array_price[3] / max_price), 1 - (array_price[4] / max_price), 1 - (array_price[5] / max_price), 1 - (array_price[6] / max_price), 1 - (array_price[7] / max_price), 1 - (array_price[8] / max_price), 1 - (array_price[9] / max_price), 1 - (array_price[10] / max_price), 1 - (array_price[11] / max_price), 1 - (array_price[12] / max_price), 1 - (array_price[13] / max_price), 1 - (array_price[14] / max_price)])
    array_discount_new.extend([array_discount[0] / max_discount, array_discount[1] / max_discount, array_discount[2] / max_discount, array_discount[3] / max_discount, array_discount[4] / max_discount, array_discount[5] / max_discount, array_discount[6] / max_discount, array_discount[7] / max_discount, array_discount[8] / max_discount, array_discount[9] / max_discount, array_discount[10] / max_discount, array_discount[11] / max_discount, array_discount[12] / max_discount, array_discount[13] / max_discount, array_discount[14] / max_discount])
    
    performance_matrix = []
    performance_matrix.extend([array_memory_new, array_price_new, array_discount_new])
    ...
    request(instance_data['instance_type'], instance_data['az'], instance_data['price'])
...
def request(instanceType, az, price):
    ...
    user_data = '''#!/bin/bash
                ...
                sudo -i
                echo "0 * * * * root stress-ng --cpu 10 --timeout 10m && sleep 120" >> /etc/cron.d/hourly
                echo "15 * * * * root stress-ng --cpu 20 --timeout 5m" >> /etc/cron.d/hourly
                echo "20 * * * * root stress-ng --cpu 120 --timeout 10m" >> /etc/cron.d/hourly
                echo "40 * * * * root stress-ng --cpu 20 --timeout 10m  && sleep 120" >> /etc/cron.d/hourly
                echo "55 * * * * root stress-ng --cpu 4 --timeout 4m" >> /etc/cron.d/hourly'''
